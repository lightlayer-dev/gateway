...
func (s *Server) registerRoutes(mux *http.ServeMux) {
	...
	// Rate limit usage
	mux.HandleFunc("GET /api/rate-limits/usage", s.handleRateLimitUsage)
...
func (s *Server) handleDiscoveryPreview(w http.ResponseWriter, r *http.Request) {
	...
	// Generate /.well-known/ai manifest.
	wellKnownAI := map[string]interface{}{
		"name":        d.Name,
	...
	agentsTxt += fmt.Sprintf("User-agent: *\nAllow: /\n")
	...
	agentCard := map[string]interface{}{
		...
		"capabilities": map[string]interface{}{
			"streaming":         cfg.Plugins.A2A.Streaming,
			"pushNotifications": cfg.Plugins.A2A.PushNotifications,
		},
		"skills": d.Capabilities,
	}

	writeJSON(w, http.StatusOK, map[string]interface{}{
		"well_known_ai":   wellKnownAI,
...
func (s *Server) handleRateLimitUsage(w http.ResponseWriter, r *http.Request) {
	...
	// Get request counts per agent within the current default window.
	window := rl.Default.Window.Duration
	if window == 0 {
		window = time.Minute
	}

	var usage []store.RateLimitUsage

	if s.store != nil {
		counts, err := s.store.GetAgentRequestCounts(time.Now().Add(-window))
		if err != nil {
			slog.Error("admin: rate limit usage query failed", "error", err)
		} else {
			for agent, count := range counts {
				limit := rl.Default.Requests
				agentWindow := window
				// Check per-agent overrides.
				if override, ok := rl.PerAgent[agent]; ok {
					limit = override.Requests
					if override.Window.Duration > 0 {
						agentWindow = override.Window.Duration
					}
				}
				pct := 0.0
				if limit > 0 {
				...
				usage = append(usage, store.RateLimitUsage{
					Agent:   agent,
					Used:    count,
					Limit:   limit,
					Window:  agentWindow.String(),
					Percent: pct,
				})
			}
		}
	...
	writeJSON(w, http.StatusOK, map[string]interface{}{
		...
		"default": map[string]interface{}{
			"requests": rl.Default.Requests,
			"window":   rl.Default.Window.Duration.String(),
		},
		"per_agent": rl.PerAgent,
		"usage":     usage,
	})
}
